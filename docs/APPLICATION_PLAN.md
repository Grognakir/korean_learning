...
- **Общее состояние:** F1-I01–F1-I17A завершены; polish `fix/cp1a-training-ui` по аудиту и ручным замечаниям CP-1A.
- **Текущая фаза:** фаза 1 — создание рабочего каркаса.
- **Текущая итерация:** fix CP-1A training UI (layout, feedback, Select, complete screen).
- **Статус текущей итерации:** `done` (код); **CP-1A: ожидает явного подтверждения**.
- **Уже сделано:** локальный training loop, draft preview 높임말 (dev-only), UI-001–UI-006, кастомные Select/form, polish сессии по приёмке.
- **Выполненные проверки:** Prettier, ESLint, 189/189 теста, typecheck и production build зелёные.
- **Сейчас работает:** preview/demo-тренировка с обновлённым layout (кнопка под упражнением, компактный feedback, исправленный complete).
- **Пока не работает:** F1-I18+ (persistence и далее) — до явного CP-1A.
- **Следующий конкретный шаг:** получить «CP-1A принят», затем F1-I18 (`feature/local-session-persistence`).
- **Блокирующие вопросы:** нужен явный CP-1A.
