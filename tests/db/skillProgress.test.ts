...
describe("skill progress and skill concept keys", () => {
  it("records mistakes without primary topic and matches skill concept keys", async () => {
    ...
    const conceptKey = "grammar:write-greeting";
    ...
    const existsLegacy = runSql(
      `select public.approved_exercise_exists_for_concept('${SAMPLE_MODULE_ID}', 'write-greeting')::text;`,
    );
    const existsSkill = runSql(
      `select public.approved_exercise_exists_for_concept('${SAMPLE_MODULE_ID}', '${conceptKey}')::text;`,
    );
    ...
    const { data: resolved, error: resolveError } = await client.rpc(
      ...
      {
        p_module_id: SAMPLE_MODULE_ID,
        p_concept_keys: [conceptKey, "write-greeting"],
      },
    );

    expect(resolveError).toBeNull();
    expect(resolved?.length).toBeGreaterThanOrEqual(1);
    expect(resolved?.some((row) => row.exercise_id === SAMPLE_EXERCISE_ID)).toBe(true);
  });
