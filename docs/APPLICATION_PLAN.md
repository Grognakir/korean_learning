...
- **Общее состояние:** фаза 1 и фаза 2 завершены; CP-9 принят пользователем 2026-08-10.
- **Текущая фаза:** пост-релиз / ранняя фаза 3 — authoring admin UI (вне исходного scope фазы 2).
- **Текущая итерация:** post-F2 admin content console (юниты, грамматика, словарь).
- **Статус текущей итерации:** локальная админка в работе в рабочем дереве; не входит в DoD фазы 2 и не блокирует CP-9.
- **Уже сделано:** PR #16–#19 смержены; migration `20260810000002` и идемпотентный curriculum upsert применены; production commit `7520673` развёрнут; CP-5…CP-9 приняты.
- **Выполненные проверки:** unit 366; content 46; integration 20; DB 24; RLS 13; E2E 34; local и Supabase production builds; bundles, performance и scans; post-merge CI зелёный.
- **Сейчас работает в production:** 16 уроков, 80 грамматик, словарь по уроку, точные фильтры тренировки, 286 approved заданий, чтение и интерактивная сессия с Enter/feedback/progress.
- **Пока не работает / ограничения:** тег и GitHub release `v0.1.0` ещё не созданы (нужен явный push/tag); Preview SSO остаётся известным ограничением среды preview; админка пока только в локальном/незакоммиченном виде.
- **Следующий конкретный шаг:** по разрешению — тег `v0.1.0`; параллельно довести и закоммитить admin authoring UI.
- **Блокирующие вопросы:** нет для CP-9; для тега — только явное разрешение на `git tag` / push.
- **Remote Supabase:** project `korean-learning` (`cyoezrdxqncroflgkyry`, `ap-northeast-2`); URL `https://cyoezrdxqncroflgkyry.supabase.co`; linked; migration `20260810000002` applied.
- **Corrective stabilization (PERF):** PERF-I01—I11 закрыты на Production.
- **Последнее обновление:** 2026-08-10.

## 1. Цель, результат и границы

Цель — создать расширяемое адаптивное веб-приложение для изучения корейского языка и выпустить проверенную программу уровня 1급. Фаза 1 создала независимый тренировочный каркас; фаза 2 добавляет 16 тематических разделов, каталог грамматики, словарь и чтение.

MVP включает навигацию по темам и грамматике, тренировки грамматики/слов/чтения, результат, повторение ошибок, прогресс, словарь и авторизацию. Уважительная речь входит в уроки 14 и 16, а не существует отдельной копией программы. Базовые упражнения полностью работоспособны без OpenAI API.

За пределами MVP без отдельного решения: подписки, социальные функции, рейтинги, чат, полноценный multi-role authoring/reviewer workflow, нативные приложения, сложная геймификация, голосовой ввод, микросервисы, Railway и автоматическая публикация ИИ-контента. Минимальная service-role админка контента начата после CP-9 (см. §25).

## 2. Подтверждения и контрольные точки

### Решения перед F1-I01

1. **Подтверждено:** архитектура и последовательные малые итерации без массового объединения.
...
7. **Подтверждено:** pnpm, ранний CI, ранний вертикальный срез и раздельные поля жизненного цикла/происхождения контента.
...
- **CP-6:** после F2-I07 — слияние текстов, 16 тем/80 грамматик, спорные формы и граница словаря.
- **CP-7:** после F2-I20 — оба каталога и три вида тренировки на всех целевых ширинах.
- **CP-8:** после F2-I22 — языковая приёмка минимального опубликованного банка.
- **CP-9:** после F2-I23 — принят пользователем 2026-08-10; тег `v0.1.0` создаётся только по отдельному явному разрешению на tag/push.
...
5. `src/modules` — server-side adapters и правила конкретных учебных областей; curriculum подключается через общие контракты.
...
Разрешённое направление: `app → features/modules/components/wrappers → lib/types`. Общий тренировочный движок принимает дискриминированные типы упражнений и реестр проверяющих, но не импортирует конкретную тему. Curriculum регистрирует контент и специфические правила через стабильные интерфейсы.
...
- **Фаза 2 — программа 1급 по темам и грамматике:** F2-I01…F2-I23, 23 итерации.
- **Всего:** фаза 1 — 34 итерации; фаза 2 — 23 итерации; корректирующие итерации не считаются заранее.
...
- **Фаза / статус:** 1 / `done`; CP-1A принят.
...
- **Входные зависимости:** F1-I17A + CP-1A.
...
- **Тесты:** missing/corrupt/expired/wrong-version storage; hydration; clear; restore remount.
...
- **Цель и зачем:** принять фазу 1 как стабильную платформу для реального учебного контента.
...
- **Следующий шаг:** F2-I01 из `docs/PHASE_2_IMPLEMENTATION_PLAN.md`.

## 12. Фаза 2 — программа 1급 по темам и грамматике

Актуальный аудит материалов: `docs/PHASE_2_CONTENT_AUDIT.md`.

Единственный исполнимый порядок F2-I01…F2-I23: `docs/PHASE_2_IMPLEMENTATION_PLAN.md`.

Канонические источники, на которых строятся данные и реализация фазы 2:

1. `docs/CURRICULUM_TOPICS.md`;
2. `docs/CURRICULUM_GRAMMAR.md`;
3. `docs/CURRICULUM_VOCABULARY.md`;
4. `docs/CURRICULUM_TEXTS.md`.

Исходные расшифровки, фотографии, заметки и производные HTML/TSV используются только для аудита происхождения. Если содержимое меняется, сначала обновляется соответствующий `CURRICULUM_*.md`, затем план и только потом JSON/seed/runtime.

Ниже сохранён архив первоначального плана только для объяснения прежних решений. Все его карточки отменены, не входят в текущую нумерацию и не должны исполняться.

### ARCHIVE-HON-01 — Формализация учебных правил
...
### ARCHIVE-HON-03 — Обычные и уважительные пары
...
### ARCHIVE-HON-04 — Проверенный базовый набор заданий
...
### ARCHIVE-HON-05 — Проверка неоднозначных переводов
...
### ARCHIVE-HON-06 — Выбор русского значения
...
### ARCHIVE-HON-07 — Выбор уважительного эквивалента
...
### ARCHIVE-HON-08 — Обратный выбор обычного слова
...
### ARCHIVE-HON-09 — Сопоставление корейских и русских слов
...
### ARCHIVE-HON-10 — Сопоставление обычных и уважительных слов
...
### ARCHIVE-HON-11 — Заполнение пропуска
...
### ARCHIVE-HON-12 — Свободный ответ на корейском
...
### ARCHIVE-HON-13 — Нормализация корейского ввода
...
### ARCHIVE-HON-14 — Несколько допустимых ответов
...
### ARCHIVE-HON-15 — Показ объяснений
...
### ARCHIVE-HON-16 — Смешанная тренировка
...
### ARCHIVE-HON-17 — Уровни сложности
...
### ARCHIVE-HON-18 — Повторение ошибок модуля
...
### ARCHIVE-HON-19 — Статистика по грамматическим конструкциям
...
### ARCHIVE-HON-20 — Автотесты всех учебных правил
...
### ARCHIVE-HON-21 — Финальная проверка корейского контента
...
### ARCHIVE-HON-22 — Стабилизация и выпуск v0.1.0
...
Этот раздел описывает реализованный baseline фазы 1. Целевые forward-only расширения фазы 2 зафиксированы в `docs/PHASE_2_IMPLEMENTATION_PLAN.md` §6; при исполнении F2-I03 они синхронизируются сюда по фактической migration.
...
**`learning_modules`** — тематический учебный модуль; в фазе 2 это один из 16 уроков 1급.
...
Интеграция не входит в фазу 2 и включается отдельным планом. Этот раздел фиксирует только архитектурные ограничения, а не заранее реализуемую AI-инфраструктуру. Базовые задания и свободные ответы работают детерминированно.
...
5. Общий engine не знает конкретную тему; curriculum не дублирует session/progress/review.
...
- Общий UI и wrappers доступны с клавиатуры, адаптивны от 320 px и не зависят от конкретной учебной темы.
...
- Временный dev-only preview 높임말 из F1-I17A удалён; фаза 2 начинает canonical curriculum без параллельного runtime-источника данных.
...
## 17. Критерии готовности программы 1급 и MVP

### Программа фазы 2 готова, когда

- Содержание ограничено заявленным 1급 и покрывает 16 тем и 80 элементов программы.
- Работают два каталога и три навыка: грамматика, слова и чтение.
...
- Статистика доступна по теме, грамматической конструкции и навыку.
- Технические тесты и 100% ручная языковая проверка опубликованного контента завершены.
- Пользователь принял CP-8; подробный минимум соответствует `PHASE_2_IMPLEMENTATION_PLAN.md` §3.2.
...
- Production smoke успешен, мониторинг первых ошибок выполнен; CP-9 принят 2026-08-10; тег `v0.1.0` — по отдельному разрешению.
...
| Curriculum 1급 | unit/grammar/source coverage | seed/repository/RLS | оба каталога и три навыка | qualified language review |
...
- Полный multi-role authoring/reviewer UI и review queue в админке; минимальная content console уже начата (§25).
...
- [x] CP-5, CP-6, CP-7, CP-8, CP-9 подтверждены (CP-9 — 2026-08-10).
...
- [ ] Оба каталога и тренировки грамматики, слов и чтения проверены.
- [ ] Опубликованы 16 тем, 80 grammar metadata records и минимальный банк из плана фазы 2.
- [ ] Все 185 записей двух исходных расшифровок учтены reconciliation manifest.
- [ ] Korean IME, Unicode normalization и допустимые ответы проверены.
- [ ] 320–2560 px с плавным sweep, keyboard-only, zoom 200%, reduced motion и контраст проверены.
...
## 25. Post-F2: минимальная админка контента

**Контекст.** В исходном плане фазы 2 (`PHASE_2_IMPLEMENTATION_PLAN.md` §3.3) админ-панель была вне scope; authoring/reviewer UI отнесён к фазе 3 (§14 п.7). После принятия CP-9 начата ранняя реализация минимальной content console — без multi-role review workflow и без редактора упражнений.

**Цель.** Дать доверенному оператору править юниты, грамматику и словарь через UI, без ручного SQL/seed для точечных правок.

**Доступ и безопасность**

- Маршруты `/admin`, `/admin/login`, `/admin/units|grammar|dictionary`.
- Отдельная cookie-сессия админа (`ADMIN_USERNAME` / `ADMIN_PASSWORD` / session secret), не learner Supabase Auth.
- Запись в БД только через service-role repository на сервере; ключ не попадает в browser graph.
- После save пишется provenance (`recordAdminProvenance`).
- Dev-only shortcut в learner header → `/admin`.

**Сущности и операции**

| Область | Список | Create/Edit | Delete |
|---|---|---|---|
| Юниты (`learning_modules`) | таблица, сортировка по `updatedAt` затем № | форма: slug → авто `unitNumber`/`sortOrder`, статус, bilingual titles, description; авто-bump `contentVersion` | блок при FK |
| Грамматика (`grammar_topics`) | пагинация по 10; фильтры q / status / unit | форма: module, pattern, titles, summaries, markdown body, status | блок при FK; сообщение со счётчиками связанных упражнений/прогресса/ошибок |
| Словарь (`dictionary_entries`) | таблица | форма lemma/POS/meanings/module/status | сначала чистит `dictionary_entry_modules` |

**UI-инварианты**

- Кастомный `Select` вместо нативного `<select>` (§4.7).
- Markdown editor + toolbar для `bodyMd` грамматики.
- Стрелка «назад к списку» слева от заголовка create/edit.
- Общий page chrome с learner: `PageContainer` default width, общие padding tokens.
- Список грамматики без колонки «Версия» (версия остаётся в форме).

**Код (основные пути)**

- `src/app/admin/**`
- `src/features/admin/**` — actions, components, `adminContentRepository`, schemas, session/env
- Env: `.env.example` — admin credentials / session secret

**Не входит в эту раннюю админку**

- CRUD упражнений, reading passages, review queue, bulk publish
- Роли reviewer/publisher, audit UI, SSO
- Публикация draft→published без явного статуса в форме (статус редактируется, но нет отдельного review workflow)

**Следующие шаги фазы 3 по authoring**

1. Довести и закоммитить текущую console.
2. Добавить упражнения / reading при необходимости.
3. Развести operator login и learner auth при необходимости multi-user.
