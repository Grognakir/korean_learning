...
- **Общее состояние:** F1-I01–F1-I17A завершены; выполнен fix `fix/cp1a-training-ui` по `docs/PHASE_1_UI_AUDIT_FINDINGS.md`.
- **Текущая фаза:** фаза 1 — создание рабочего каркаса.
- **Текущая итерация:** fix CP-1A training UI; код готов к повторной приёмке.
- **Статус текущей итерации:** `done` (код); **CP-1A: ожидает повторного явного подтверждения**.
- **Уже сделано:** локальный training loop, draft preview 높임말 (dev-only), исправления UI-001–UI-006.
- **Выполненные проверки:** Prettier, ESLint, 186/186 теста в 46 файлах, typecheck и production build зелёные.
- **Сейчас работает:** sample/preview-тренировка с исправленной мобильной панелью действий, стабильным прогрессом и Enter-submit.
- **Пока не работает:** F1-I18+ (persistence и далее) — до явного CP-1A.
- **Следующий конкретный шаг:** повторная визуальная приёмка desktop/mobile; F1-I18 запрещена до «CP-1A принят».
- **Блокирующие вопросы:** нужен явный CP-1A после проверки исправлений.
