...
- **Общее состояние:** F1-I01–F1-I17A завершены; остаток фазы 1 подробно зафиксирован в `docs/PHASE_1_REMAINING_IMPLEMENTATION_PLAN.md`.
- **Текущая фаза:** фаза 1 — создание рабочего каркаса.
- **Текущая итерация:** F1-I17A — ранний vertical slice 높임말; завершена, ожидает CP-1A.
- **Статус текущей итерации:** `done` (код); **CP-1A: blocked / ожидает пользователя**.
- **Уже сделано:** локальный training loop (evaluator + session engine + UI), draft preview 높임말 (11 упражнений, 2 темы) только в development composition.
- **Выполненные проверки:** Prettier, ESLint, 177/177 теста в 46 файлах, typecheck и production build зелёные; production не публикует `/topics/honorifics`.
- **Сейчас работает:** sample-тренировка и (только в development) draft preview 높임말 на общем UI.
- **Пока не работает:** локальное возобновление, экран результата, application states, E2E, Supabase и далее — до CP-1A.
- **Следующий конкретный шаг:** пользователь проходит 1–2 короткие тренировки на desktop/mobile и принимает CP-1A; F1-I18 запрещена до явного принятия.
- **Блокирующие вопросы:** нужен явный CP-1A (пригодность общего UI на draft 높임말).
- **Решения, требующие подтверждения:** CP-1A (сейчас), затем CP-2/CP-3/CP-4/CP-5 позже.
...
- **Фаза / статус:** 1 / `done` (код); ожидает CP-1A.
