...
Проверка публичного Production после merge `43daf15` показала, что окружения расходятся с требованиями PERF-I00 шаг 3 и PERF-I01 шаг 4.

Production отвечает `200` на всех маршрутах, но отдаёт локальные демо-фикстуры вместо контента Supabase:

1. в каталоге единственная карточка «Первые шаги в корейском» — это локальный `sample-module`;
2. `/topics/honorifics` → `404`, хотя в Supabase опубликован именно `honorifics`;
3. `/training/honorifics-preview` → `404`.

Источник выводится из кода однозначно: `parseDeploymentSupabaseEnv` пропускает валидацию только при явном `CONTENT_SOURCE=local`, иначе на Vercel требует все три Supabase-переменные и роняет сборку. Сборка прошла, а при заданных ключах и `CONTENT_SOURCE` в значении `supabase` или пустом значении `resolveContentSource()` вернул бы `supabase`. Следовательно, в Production-окружении задано `CONTENT_SOURCE=local`.

Практические следствия:

1. Публичный Production показывает демо-контент, а не учебные модули.
2. Проблема ключа Supabase на Production замаскирована; она проявляется только на Preview, где используется настоящий источник.
3. Порядок исправления: сначала подтвердить рабочий ключ на Preview, затем переключить Production на `CONTENT_SOURCE=supabase`. Обратный порядок оставит Production живым, но заменит весь контент на «Сервис недоступен».
