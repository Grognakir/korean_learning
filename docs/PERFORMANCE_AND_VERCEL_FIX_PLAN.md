...
1. Чистый `next build` с реальными ключами и `CONTENT_SOURCE=supabase` проходит, то есть код и креды исправны — отличается только окружение Vercel. (Первая редакция утверждала, что эта сборка пререндерит `/topics/honorifics`; утверждение ошибочно и исправлено в PERF-I11.)
...
Диагностика Preview 2026-08-09 заняла шесть сборок и не дала ответа, потому что ни один артефакт не называет источник:

1. Каталоги обоих источников совпадают: и локальные production-фикстуры, и удалённый Supabase публикуют ровно один модуль `sample-module`. Поэтому строка `/topics/sample-module` в таблице маршрутов сборки одинаково согласуется с обоими источниками.
2. `404` на `/topics/honorifics` тоже ничего не различает: модуль отсутствует в обоих источниках, так как это локальный draft для `NODE_ENV=development`, намеренно не импортированный в Supabase (`supabase/seed.sql`).
3. `prebuild` (`scripts/validate-deployment-env.ts`) при `CONTENT_SOURCE=local` молча выходит и ничего не печатает, а при `supabase` печатает только сообщения об ошибке.
4. После PERF-I10 сбой хранилища превращается в значение `unavailable` без единой записи в логах, поэтому причина деградации не видна ни в build logs, ни в runtime logs.

Побочный результат разбора: записи этого документа о том, что в Supabase опубликован `honorifics`, фактически неверны. Проверка REST-запросом к удалённому проекту публикуемым ключом вернула единственную строку `{"slug":"sample-module","status":"published"}`. Ошибочные утверждения исправлены в этой же итерации.

#### 2. Реализация

1. `src/modules/contentSource.ts` — `describeContentSource(env)`: одна строка с фактически выбранным источником; для Supabase добавляется host проекта, для локального — значение `CONTENT_SOURCE` либо `unset`. Секреты не печатаются.
2. `scripts/validate-deployment-env.ts` — печатает эту строку после успешной валидации, поэтому каждый build log Vercel начинается с явного указания источника.
3. `src/modules/cachedLearningContent.ts` — `readContent` логирует сообщение `LearningContentError` перед тем, как вернуть `unavailable`. Контракт PERF-I10 не меняется: ошибка по-прежнему не пересекает границу `"use cache"`.

#### 3. Не делать

1. Не добавлять в `prebuild` сетевой запрос к Supabase: сборка не должна зависеть от доступности хранилища, это прямо противоречит PERF-I10.
2. Не печатать значения ключей и не выводить `SUPABASE_SECRET_KEY` даже частично.
3. Не менять выбор источника и не трогать поведение Proxy, `generateStaticParams` и `cacheLife`.

#### 4. Тесты и фактические результаты

Добавлено: `src/modules/contentSource.test.ts` (host проекта для Supabase, явный и отсутствующий `CONTENT_SOURCE` для локальных фикстур, непригодный URL не роняет строку, ключи не попадают в вывод) и проверка в `src/modules/cachedLearningContent.test.ts`, что деградация оставляет запись `Learning content unavailable: <причина>`.

До / после для build log:

| Ситуация                              | До         | После                                                   |
| ------------------------------------- | ---------- | ------------------------------------------------------- |
| Локальная сборка без `CONTENT_SOURCE` | нет строки | `Content source: local fixtures (CONTENT_SOURCE=unset)` |
| Сборка с Supabase                     | нет строки | `Content source: supabase (project-ref.supabase.co)`    |
| Недоступное хранилище в runtime       | нет записи | `Learning content unavailable: <сообщение хранилища>`   |
...
| ----------------------------------- | ---------------------------------------------------------------------------- |
| `format:check`, `lint`, `typecheck` | green                                                                        |
| Unit                                | 303 passed (было 297)                                                        |
| Integration                         | 17 passed                                                                    |
| Playwright desktop + mobile         | 20 passed                                                                    |
| `next build`                        | prerendered маршруты и `5m / 1d` без изменений                               |
| Bundle budgets                      | без изменений: `/topics`, `/training`, `/training/[sessionId]` — 159 KB gzip |

Отдельно проверено на чистом окружении, что при `VERCEL=1` и отсутствующем `SUPABASE_SECRET_KEY` prebuild по-прежнему завершается кодом 1 и сообщением `Supabase deployment environment is missing required keys: SUPABASE_SECRET_KEY`, то есть новая печать не маскирует валидацию.

Отклонение от раздела 7 условий итерации: gate выполнен на Node.js `v22.15.0`, потому что требуемый `24.18.0` на машине не установлен и менеджера версий нет. Расхождение согласовано с пользователем; Vercel собирает на Node 24 независимо.
...
3. Все unit и integration tests — текущий baseline после PERF-I11: 303 unit + 17 integration.
4. Production build.
5. E2E:
