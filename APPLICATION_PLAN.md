...
- **Общее состояние:** F1-I01–F1-I05 завершены; минимальный source tree имеет содержательные публичные границы.
- **Текущая фаза:** фаза 1 — создание рабочего каркаса.
- **Текущая итерация:** F1-I05 — каркас каталогов и публичные границы.
- **Статус текущей итерации:** `done`.
- **Уже сделано:** добавлены содержательные слои `types`, `constants` и `lib/utilities`; публичные `index.ts` экспортируют только поддерживаемые контракты; metadata и главная страница используют общие app-константы; добавлена протестированная утилита `classNames`.
- **Выполненные проверки:** Prettier check, ESLint без предупреждений, 2/2 теста, `next typegen`, строгий `tsc --noEmit` и production build проходят на Node.js 24.18.0; алиас `@/` проверен production- и test-импортами; пустых каталогов и циклических направлений импортов нет.
- **Сейчас работает:** минимальная страница `/`, App Router, строгий TypeScript, quality/test harness, production build, CI и первые публичные границы source tree.
...
- **Следующий конкретный шаг:** F1-I06 — добавить дизайн-токены, reset и глобальные стили с доступным focus/reduced-motion фундаментом.
