...
- **Общее состояние:** F1-I01 завершена; репозиторий подготовлен к инициализации приложения.
- **Текущая фаза:** фаза 1 — создание рабочего каркаса.
- **Текущая итерация:** F1-I01 — окружение и подготовка репозитория.
- **Статус текущей итерации:** `done`.
- **Уже сделано:** создана ветка `chore/environment-and-repository`; зафиксированы Node.js 24.18.0 LTS и поддерживаемый Vercel pnpm 10.34.5; создан безопасный `.gitignore`; старый неотслеживаемый `package.json` заменён приватным manifest приложения; подготовлен отдельный коммит F1-I01.
- **Выполненные проверки:** package JSON корректен; `.nvmrc` содержит 24.18.0; Corepack запускает pnpm 10.34.5; `.env*`, Next.js, test, Supabase, Vercel и локальные Obsidian artifacts игнорируются, а `.env.example` разрешён; `git diff --check` проходит; ветка и границы staging проверены. TypeScript, ESLint, тесты и production build неприменимы до F1-I02/F1-I04.
- **Сейчас работает:** воспроизводимая конфигурация Node.js/pnpm и безопасные правила репозитория; рабочая ветка готова к генерации Next.js.
- **Пока не работает:** маршруты, UI, тренировки, тесты, Supabase, авторизация, прогресс, модуль 높임말, CI и деплой.
- **Следующий конкретный шаг:** F1-I02 — активировать Node.js 24.18.0 для команд проекта и инициализировать Next.js без Tailwind в корне.
- **Блокирующие вопросы:** нет; локальный системный Node.js 22.15.0 не менялся, поэтому F1-I02 сначала активирует проектный Node.js 24.18.0.
