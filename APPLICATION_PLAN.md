...
- **Общее состояние:** F1-I01–F1-I11 завершены; CP-1 готова к пользовательской визуальной проверке.
- **Текущая фаза:** фаза 1 — создание рабочего каркаса.
- **Текущая итерация:** F1-I11 — мобильная адаптация оболочки.
- **Статус текущей итерации:** `done`.
- **Уже сделано:** доступны базовые UI/feedback-компоненты, общая оболочка, PageContainer/ContentSection/TrainingShell, девять пользовательских маршрутов, desktop/mobile navigation, active-state, safe-area и адаптивные брейкпоинты.
- **Выполненные проверки:** Prettier, ESLint, 39/39 тестов в 25 файлах, `next typegen`, строгий `tsc --noEmit` и production build проходят на Node.js 24.18.0; browser smoke охватывает все маршруты, back/forward, 404 и матрицу 320/375/768/1280 px, landscape и эквивалент 200% zoom без overflow; минимальная высота основных целей — 44 px.
- **Сейчас работает:** навигируемая mobile-first оболочка MVP, доступные примитивы и системная обратная связь, статические и динамические route placeholders, метаданные и production build.
- **Пока не работает:** доменные модели модулей/упражнений, реальные тренировки, E2E, Supabase, авторизация, сохраняемый прогресс, модуль 높임말, внешний CI run и деплой.
- **Следующий конкретный шаг:** пользовательская проверка CP-1, затем F1-I12 — универсальная модель учебных модулей.
- **Блокирующие вопросы:** нет.
- **Решения, требующие подтверждения:** принять CP-1 или перечислить визуальные корректировки; внешние Vercel/Supabase действия выполняются на предусмотренных контрольных точках.
